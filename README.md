...
- **라벨 정의 자체가 느슨함.** 원본에 딸린 탐지용 바운딩박스 라벨(1,800장 전부 존재, 박스 4,189개)과 대조해보니 **123장(6.8%)이 파일명 클래스와 다른 종류의 결함 박스를 함께 가지고 있었음** — 남의 클래스 박스 188개, 전체 박스의 4.5%. 클래스별 편차도 큼: `scratches` **16.7%**, `pitted_surface` 13.0%, `patches` 9.7%인 반면 `inclusion`·`rolled-in_scale`은 **0%**. 실제로 세어보니 **test 270장 중 23장(8.5%)이 정답이 하나가 아닌데도 단일 라벨로 채점됐음.** 즉 test 100%는 "데이터가 쉬웠다"만이 아니라 **"문제 정의가 느슨했다"의 결과이기도 함** — 다중 라벨 분류나 탐지로 문제를 다시 세워야 함 (위 수치는 `audit_data.py`로 재현 가능)

...
def mcnemar_exact(b, c):
    """맥니마 정확검정 — 같은 시험지를 푼 두 모델의 차이가 우연인지 판정.

    같은 270장을 두 모델이 풀면 '짝지은 비교'라서, 전체 정답 수가 아니라
    서로 엇갈린 문제만 봄:
      b = 모델A만 맞힌 문제 수, c = 모델B만 맞힌 문제 수
    둘 다 맞거나 둘 다 틀린 문제는 차이에 대한 정보가 없어서 버림.
    귀무가설(두 모델이 같다)이면 엇갈린 문제는 동전던지기(p=0.5)와 같음 —
    그래서 이항분포로 양측 p값을 정확히 계산함. p가 작으면 차이가 진짜임.
    """
    n = b + c
    if n == 0:
        return 1.0                              # 엇갈린 문제가 없으면 차이 증거도 없음
    k = min(b, c)
    # P(X <= k), X ~ Binomial(n, 0.5) 를 조합으로 직접 합산
    tail = sum(math.comb(n, i) for i in range(k + 1)) / (2 ** n)
    return min(1.0, 2 * tail)                   # 양측 = 한쪽 꼬리의 2배 (1을 넘지 않게)
